...
import styles from './NandUtsavMaker.module.css';
...
const DEFAULT_FRAME = { cx: 0.705, cy: 0.645, r: 0.155 };
const DEFAULT_NAME = { cx: 0.30, cy: 0.775, fontPct: 0.025, maxName: 18 };

export default function NandUtsavMaker() {
  ...
  useEffect(() => {
    axios.get('/api/admin/nand-utsav-settings')
      .then(({ data }) => {
        setFRAME({
          cx: data.nuFrameCX ?? DEFAULT_FRAME.cx,
          cy: data.nuFrameCY ?? DEFAULT_FRAME.cy,
          r:  data.nuFrameR  ?? DEFAULT_FRAME.r,
        });
        setNAME({
          cx:      data.nuNameCX      ?? DEFAULT_NAME.cx,
          cy:      data.nuNameCY      ?? DEFAULT_NAME.cy,
          fontPct: data.nuNameFontPct ?? DEFAULT_NAME.fontPct,
          maxName: data.nuMaxName     ?? DEFAULT_NAME.maxName,
        });
      })
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      console.error('NandUtsav template load failed');
      setLoading(false);
    };
      ...
      <header className={styles.header}>
        <div className={styles.headerInner}>
          ...
          <div className={styles.headerText}>
            <h1 className={styles.headerTitle}>પ્રેમાળ જીવદયા ટ્રસ્ટ — નંદ ઉત્સવ</h1>
            <p className={styles.headerSub}>Create Your Nand Utsav Poster · નંદ ઉત્સવ પોસ્ટર બનાવો</p>
          </div>
        </div>
      </header>
