...
export default function NandUtsavLayout() {
  ...
  // Load NandUtsav.png for preview
  useEffect(() => {
    const img = new Image();
    img.onload = () => {
      const SIZE = 480;
  ...
  const fetchLayout = async () => {
    try {
      const { data } = await axios.get('/api/admin/nand-utsav-settings');
      setLayoutForm({
        nuFrameCX:     String(data.nuFrameCX),
        nuFrameCY:     String(data.nuFrameCY),
        nuFrameR:      String(data.nuFrameR),
        nuNameCX:      String(data.nuNameCX),
        nuNameCY:      String(data.nuNameCY),
        nuNameFontPct: String(data.nuNameFontPct),
        nuMaxName:     String(data.nuMaxName),
      });
    } catch { /* keep defaults */ }
  };
  ...
  const drawPreview = useCallback((form) => {
    ...
    const fcx = parseFloat(form.nuFrameCX) || 0;
    const fcy = parseFloat(form.nuFrameCY) || 0;
    const fr  = parseFloat(form.nuFrameR) || 0;
    const ncx = parseFloat(form.nuNameCX) || 0;
    const ncy = parseFloat(form.nuNameCY) || 0;
    const nfp = parseFloat(form.nuNameFontPct) || 0.025;
  ...
  const hitTest = (mx, my, form, cw, ch) => {
    const fcx = parseFloat(form.nuFrameCX) * cw;
    const fcy = parseFloat(form.nuFrameCY) * ch;
    const fr  = parseFloat(form.nuFrameR) * cw;
    const ncx = parseFloat(form.nuNameCX) * cw;
    const ncy = parseFloat(form.nuNameCY) * ch;
  ...
  const onPreviewMove = (e) => {
    ...
    if (type === 'frame-move') {
      n.nuFrameCX = fmt(cl(parseFloat(sl.nuFrameCX) + dx / cw));
      n.nuFrameCY = fmt(cl(parseFloat(sl.nuFrameCY) + dy / ch));
    } else if (type === 'frame-top' || type === 'frame-bottom') {
      const delta = type === 'frame-top' ? -dy : dy;
      n.nuFrameR = fmt(cl(parseFloat(sl.nuFrameR) + delta / cw, 0.01, 0.5));
    } else if (type === 'frame-left' || type === 'frame-right') {
      const delta = type === 'frame-left' ? -dx : dx;
      n.nuFrameR = fmt(cl(parseFloat(sl.nuFrameR) + delta / cw, 0.01, 0.5));
    } else if (type === 'name-move') {
      n.nuNameCX = fmt(cl(parseFloat(sl.nuNameCX) + dx / cw));
      n.nuNameCY = fmt(cl(parseFloat(sl.nuNameCY) + dy / ch));
    }
  ...
  const handleLayoutSave = async () => {
    ...
    try {
      await axios.put('/api/admin/nand-utsav-settings', {
        nuFrameCX:     parseFloat(layoutForm.nuFrameCX),
        nuFrameCY:     parseFloat(layoutForm.nuFrameCY),
        nuFrameR:      parseFloat(layoutForm.nuFrameR),
        nuNameCX:      parseFloat(layoutForm.nuNameCX),
        nuNameCY:      parseFloat(layoutForm.nuNameCY),
        nuNameFontPct: parseFloat(layoutForm.nuNameFontPct),
        nuMaxName:     parseInt(layoutForm.nuMaxName, 10),
      }, {
    ...
    return (
      <div className={styles.loginPage}>
        <form className={styles.loginCard} onSubmit={handleLogin}>
          ...
          <h2 className={styles.loginTitle}>Nand Utsav Layout</h2>
          <p className={styles.loginSub}>Configure Nand Utsav Poster Layout</p>
  ...
  const layoutFields = [
    {
      section: 'Photo Frame (Circle)', fields: [
        { key: 'nuFrameCX', label: 'Center X (0–1)', hint: 'Horizontal center of frame' },
        { key: 'nuFrameCY', label: 'Center Y (0–1)', hint: 'Vertical center of frame' },
        { key: 'nuFrameR', label: 'Radius (0–1)', hint: 'Radius as fraction of width' },
      ]
    },
    {
      section: 'Name Text', fields: [
        { key: 'nuNameCX', label: 'Center X (0–1)', hint: 'Horizontal center of name' },
        { key: 'nuNameCY', label: 'Center Y (0–1)', hint: 'Vertical center of name' },
        { key: 'nuNameFontPct', label: 'Font size (0–1)', hint: 'Font size as fraction of width' },
        { key: 'nuMaxName', label: 'Max chars', hint: 'Max name length', integer: true },
      ]
    },
  ];
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          <h1 className={styles.headerTitle}>Nand Utsav Layout</h1>
          <span className={styles.headerSub}>Configure Nand Utsav Poster Layout</span>
        </div>
